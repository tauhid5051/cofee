...
class Reports extends MY_Controller
{
    ...
    public function getProductsReport($pdf = null, $xls = null)
    {
        ...
        //  $this->print_arrays($this->datatables);
    ...
    public function collections()
    {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers']    = $this->site->getAllCompanies('biller');
        $bc                       = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('collections_report')]];
        $meta                     = ['page_title' => lang('collections_report'), 'bc' => $bc];

        if ($this->input->post('start_date') && $this->input->post('end_date') && $this->input->post('user')[0]) {
            ...
            $query = $this->db->query("SELECT sma_companies.name, sma_sales.date, sma_sale_items.product_id, sma_companies.id customer_id, sma_products.second_name, sma_sale_items.product_name, ROUND( sma_sale_items.net_unit_price, 2 ) net_unit_price, ROUND(SUM(sma_sale_items.quantity), 0) Total_quantity, ROUND(SUM(sma_sale_items.subtotal), 0) Sub_total FROM sma_sale_items LEFT JOIN sma_sales ON sma_sales.id = sma_sale_items.sale_id LEFT JOIN sma_companies ON sma_companies.id = sma_sales.customer_id LEFT JOIN sma_products ON sma_products.id = sma_sale_items.product_id WHERE sma_sales.customer_id in ($str)AND sma_sales.date BETWEEN '$start_date' AND '$end_date' GROUP BY  sma_companies.name, sma_sale_items.product_name, sma_sale_items.product_id, sma_companies.id ORDER BY `name`, product_name");
        } else {
            $query = $this->db->query("SELECT sma_companies.name, sma_sale_items.product_id, sma_companies.id customer_id, sma_products.second_name, sma_sale_items.product_name, ROUND(sma_sale_items.net_unit_price,2) net_unit_price, ROUND(SUM(sma_sale_items.quantity),0) Total_quantity , ROUND(SUM(sma_sale_items.subtotal),0) Sub_total FROM sma_sale_items LEFT JOIN sma_sales ON sma_sales.id = sma_sale_items.sale_id LEFT JOIN sma_companies ON sma_companies.id = sma_sales.customer_id LEFT JOIN sma_products ON sma_products.id = sma_sale_items.product_id GROUP BY sma_companies.name, sma_sale_items.product_name, sma_sale_items.net_unit_price, sma_sale_items.product_id, sma_companies.id ORDER BY `name`, product_name");
        }
        ...
        $this->page_construct('reports/collections', $meta, $this->data);
    }
    ...
    public function UserWiseCollection()
    {
        ...
        if ($customer_group) {
            $this->db->where('sma_companies.`customer_group_id`', $customer_group);
        }
    ...
    public function productReport()
    {
        ...
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('Product Report')]];
        $meta = ['page_title' => lang('Product Report'), 'bc' => $bc];
